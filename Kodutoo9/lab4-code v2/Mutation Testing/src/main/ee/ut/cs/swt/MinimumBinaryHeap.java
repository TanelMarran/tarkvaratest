...
public class MinimumBinaryHeap {
	...
	public boolean remove(int element) {
		int index = heap.indexOf(element); //BUG! There was no check to see if the returned index is -1 aka element not found.
		if(index == -1) {
			return false;
		}
		if (index == heap.size()-1) {
			heap.remove(heap.size()-1);
			return true; //BUG! This was "return false;" before.
		}
